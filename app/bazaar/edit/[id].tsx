...
export default function BazaarEditScreen() {
  ...
  const loadPost = async (postId: string) => {
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      Alert.alert('Unable to load listing', error.message);
    ...
    const { data: profile, error: profileError } = await supabase
      ...
      .maybeSingle();
    if (profileError) {
      Alert.alert('Unable to verify permissions', 'Please try again in a moment.');
      router.back();
      return;
    }
    const isAdmin = !!profile?.is_admin;
    if (data.user_id !== user.id && !isAdmin) {
      Alert.alert('Not allowed', 'Only the listing owner or an admin can edit this post.');
      router.back();
      return;
    }
    setPost(data);
    ...
    setCurrency((data.currency as 'CAD' | 'USD' | 'EUR') ?? 'CAD');
    setPageLoading(false);
  };
  ...
  const handleSubmit = async () => {
    if (!user || !post) return;
    if (!title.trim()) {
      Alert.alert('Missing info', 'Please add a listing title.');
      return;
    }
    ...
    if (!contact.trim()) {
      Alert.alert('Missing info', 'Please add a contact method.');
      return;
    }
    setLoading(true);
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          price: Number(price),
          currency,
          photo_url: photoUrl ?? post.photo_url,
        })
        .eq('id', post.id);
      if (error) {
        throw error;
      ...
      Alert.alert('Unable to update listing', (error as Error).message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
          ...
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            ...
            <View style={styles.headerText}>
              <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Edit Listing</Text>
              <Text style={[styles.headerSubtitle, { color: colors.textMuted }]}>Update your listing details</Text>
            </View>
          </View>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  <Text style={[styles.submitButtonText, { color: colors.white }]}>Save Changes</Text>
                </>
              )}
            </Pressable>
